...
class MealTableViewController: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    func searchBar() {
        ...
        searchController.searchBar.scopeButtonTitles = ["ALL", "TỞM", "Ngon", "Rẻ", "OTHER"]
        searchController.searchBar.delegate = self
    }
    
    func updateSearchResults(for searchController: UISearchController) {
        let searchText = searchController.searchBar
        let scope = searchText.scopeButtonTitles![searchText.selectedScopeButtonIndex]
    ...
    func filterContentForSearch(_ searchText: String, scope: String = "ALL") {
        filterMeals = meals.filter({ (meal: Meal) -> Bool in
            let doesCatogeryMath = (scope == "ALL")
            if isSearchBarEmpty() {
                return doesCatogeryMath
            } else {
                
                return doesCatogeryMath && meal.name.lowercased().contains(searchText.lowercased())
            }
        })
    ...
    private func loadSampleMeals() {
        ...
        guard let meal1 = Meal(name: "Lòng Lợn", photo: photo1, rating: 4) else {
            fatalError("Unable to instantiate meal1")
        }
        
        guard let meal2 = Meal(name: "Tiết Canh", photo: photo2, rating: 5) else {
            fatalError("Unable to instantiate meal2")
        }
        
        guard let meal3 = Meal(name: "Trứng vịt lộn", photo: photo3, rating: 3) else {
            fatalError("Unable to instantiate meal2")
        }
